...
def test_gpt4all_plugin():
    ...
    if success:
        ...
        print("[FAIL] Plugin initialization failed (make sure GPT4All is running on localhost:4891)")
        return False
    ...
    if list_result.get("success") and list_result.get("models"):
        first_model = list_result["models"][0].get("id")
        if first_model:
...
if __name__ == "__main__":
    try:
        test_gpt4all_plugin()
        print("\n[SUCCESS] GPT4All plugin test completed!")
        print("\nNote: Some tests may be skipped if GPT4All server is not running or no models are loaded.")
