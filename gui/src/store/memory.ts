/**
 * Memory
 * Implement `gui/src/store/memory.ts` to surface contextual history within the chat interface.
...
export class Memory {
    ...
    async setup(): Promise<boolean> {
        try {
            ...
            console.log('memory setup completed');
            return true;
        } catch (error) {
    ...
    async execute(options: ExecutionOptions = {}): Promise<ExecutionResult> {
        if (!this.initialized) {
            throw new Error('memory not initialized. Call setup() first.');
        }
