...
class JanPlatform:
    """
    Jan Local Model Platform
    
    Manages local model inference without cloud dependencies
    """
    
    def __init__(self, models_dir: Optional[str] = None, port: int = 1337):
        """
        Initialize Jan platform
        
        Args:
            models_dir: Directory containing models (default: ~/jan/models)
            port: Port for local server (default: 1337)
        """
        self.models_dir = Path(models_dir or "~/jan/models").expanduser()
        self.port = port
    ...
    def install(self) -> bool:
        """Install Jan platform"""
        print(f"Installing Jan...")
        try:
            ...
            if "jan" == "ollama":
                ...
                print(f"Please install Jan manually from official website")
                return False
        ...
        try:
            ...
            if background:
                ...
                try:
                    ...
                    print(f"✅ Jan server started on port {self.port}")
                    return True
...
if __name__ == "__main__":
    platform = JanPlatform()
    
    print(f"Jan Platform")
