...
Agent: "Phase 2 complete. Starting Phase 3: Testing..."
[Creates comprehensive test suite]
Agent: "Phase 3 complete. Starting Phase 4: Documentation..."
[Updates all docs]
Agent: "Phase 4 complete. Running final validation..."
[Runs all tests, builds, checks]
...
7. **"Test Skipping"** - Don't skip tests to save time. Tests are mandatory.
8. **"Documentation Deferral"** - Don't skip docs. Update them as you go.
...
┌─────────────────────────────────────────────────────────────────┐
│                     SECURITY LAYER                               │
│  • Sandbox (windows_ai/security/sandbox.py)                    │
│  • Permissions (windows_ai/security/permissions.py)            │
│  • Guardrails (windows_ai/security/guardrails.py)              │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│                   SYSTEM RESOURCES                               │
...
SecurityScanningManager     # Vulnerability scanning, security analysis
...
All contributions must achieve **60%+ test coverage**. Use pytest markers for organization:
...
**Test markers**: `unit`, `integration`, `e2e`, `slow`, `benchmark`, `security`, `critical`, `skip_ci`

Configuration in `pytest.ini` enforces coverage threshold and async mode.
...
Windows AI includes a multi-level sandbox in `windows_ai/security/sandbox.py`:

**Levels**: `NONE`, `MINIMAL`, `STANDARD` (default), `STRICT`, `MAXIMUM`

Each level progressively restricts:
...
Configure via `SandboxConfig` when initializing the orchestrator.
...
├── security/       # Sandbox, permissions, guardrails
...
#### Test Coverage Requirements (STRICTLY ENFORCED)

**Minimum Coverage**: 60% (enforced by pytest.ini)
**Target Coverage**: 80%+
**Critical Paths Coverage**: 95%+

#### How to Write Tests for Windows AI
...
async def test_plugin_loading_and_execution():
    ...
    # Execute plugin
    result = await plugin.execute(test_param="value")
...
async def test_complete_chat_workflow():
    """Test entire chat workflow from start to finish"""
    # Initialize system
    ai = WindowsAI()
    await ai.initialize()
...
@pytest.mark.asyncio
async def test_sandbox_blocks_dangerous_operations():
    """Test that sandbox blocks dangerous file operations"""
    ...
    with pytest.raises(PermissionError):
        await sandbox.execute_command("del /s C:\\*")
```
...
pytest tests/unit/test_orchestrator.py::test_orchestrator_initialization
...
# Run with coverage
pytest --cov=windows_ai                      # Basic coverage
pytest --cov=windows_ai --cov-report=html   # HTML report (htmlcov/index.html)
pytest --cov=windows_ai --cov-report=term-missing  # Show missing lines
pytest --cov=windows_ai --cov-report=xml    # XML report (for CI)

# Run tests matching pattern
pytest -k "test_orchestrator"    # All tests with "orchestrator" in name
pytest -k "not slow"             # Exclude slow tests

# Run with specific Python warnings
pytest -W error                  # Treat warnings as errors
pytest -W ignore::DeprecationWarning  # Ignore deprecation warnings
...
security:
  sandbox:
    enabled: true
    level: "standard"  # none, minimal, standard, strict, maximum
  rate_limiting:
    enabled: true
...
4. ❌ Stop after writing some tests
5. ❌ Ask "should I continue?" (answer is always YES)
6. ❌ Wait for user feedback mid-task
7. ❌ Leave TODO comments in code
8. ❌ Create stub implementations
9. ❌ Skip test writing
10. ❌ Declare victory before 100% complete
...
7. ✅ Write comprehensive tests (60%+ coverage)
8. ✅ Update documentation as you go
9. ✅ Verify functionality before marking complete
10. ✅ Only stop at 100% verified completion
...
- Execute tests for entire modules at once
- Build incrementally and verify frequently
- Don't over-analyze - implement, test, move forward

**Maintain Momentum**:
- No long pauses between tasks
- Don't re-read files unnecessarily  
- Trust your implementation - test to verify
...
- **No placeholders**: All features are production-ready implementations
- **Cross-platform**: Designed for Windows but Python backend is cross-platform
- **Extensible**: Plugin system allows easy addition of new capabilities
- **API-first**: Everything accessible via REST API at http://127.0.0.1:8010/docs
- **CONTINUOUS EXECUTION**: The #1 rule above all - never stop until 100% complete
