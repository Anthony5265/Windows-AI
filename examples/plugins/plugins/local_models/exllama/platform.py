"""
ExLlama Platform Implementation
...
class ExLlamaPlatform:
    """
    ExLlama Local Model Platform
    
    Manages local model inference without cloud dependencies
    """
    
    def __init__(self, models_dir: Optional[str] = None, port: int = 5000):
        """
        Initialize ExLlama platform
        
        Args:
            models_dir: Directory containing models (default: ~/exllama/models)
            port: Port for local server (default: 5000)
        """
        self.models_dir = Path(models_dir or "~/exllama/models").expanduser()
        self.port = port
        self.base_url = f"http://localhost:{port}"
        self.process = None
        self.executable = "exllama"
    
    def is_installed(self) -> bool:
        """Check if ExLlama is installed"""
        try:
    ...
    def install(self) -> bool:
        """Install ExLlama platform"""
        print(f"Installing ExLlama...")
        try:
            ...
            if "exllama" == "ollama":
                ...
                print(f"Please install ExLlama manually from official website")
                return False
        ...
        try:
            ...
            if background:
                ...
                try:
                    ...
                    print(f"✅ ExLlama server started on port {self.port}")
                    return True
    ...
    def stop_server(self):
        ...
        if self.process:
            ...
            self.process.wait()
            print(f"✅ ExLlama server stopped")
...
if __name__ == "__main__":
    platform = ExLlamaPlatform()
    
    print(f"ExLlama Platform")
