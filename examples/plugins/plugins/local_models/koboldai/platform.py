"""
KoboldAI Platform Implementation
...
class KoboldAIPlatform:
    """
    KoboldAI Local Model Platform
    
    Manages local model inference without cloud dependencies
    """
    
    def __init__(self, models_dir: Optional[str] = None, port: int = 5001):
        """
        Initialize KoboldAI platform
        
        Args:
            models_dir: Directory containing models (default: ~/KoboldAI/models)
            port: Port for local server (default: 5001)
        """
        self.models_dir = Path(models_dir or "~/KoboldAI/models").expanduser()
        self.port = port
        self.base_url = f"http://localhost:{port}"
        self.process = None
        self.executable = "koboldcpp"
    
    def is_installed(self) -> bool:
        """Check if KoboldAI is installed"""
        try:
    ...
    def install(self) -> bool:
        """Install KoboldAI platform"""
        print(f"Installing KoboldAI...")
        try:
            ...
            if "koboldai" == "ollama":
                ...
                print(f"Please install KoboldAI manually from official website")
                return False
        ...
        try:
            ...
            if background:
                ...
                try:
                    ...
                    print(f"✅ KoboldAI server started on port {self.port}")
                    return True
    ...
    def stop_server(self):
        ...
        if self.process:
            ...
            self.process.wait()
            print(f"✅ KoboldAI server stopped")
