...
class vLLMPlatform:
    ...
    def __init__(self, models_dir: Optional[str] = None, port: int = 8000):
        """
        Initialize vLLM platform
        
        Args:
            models_dir: Directory containing models (default: ~/.cache/huggingface)
            port: Port for local server (default: 8000)
        """
        self.models_dir = Path(models_dir or "~/.cache/huggingface").expanduser()
        self.port = port
        self.base_url = f"http://localhost:{port}"
        self.process = None
        self.executable = "vllm"
    ...
    def install(self) -> bool:
        """Install vLLM platform"""
        print(f"Installing vLLM...")
        try:
            ...
            if "vllm" == "ollama":
                ...
                print(f"Please install vLLM manually from official website")
                return False
