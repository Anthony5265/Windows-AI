"""
Mailgun - Production Implementation
Email API
"""
...
class MailgunPlugin(IntegrationPlugin):
    def __init__(self):
        metadata = PluginMetadata(
            ...
            tags=["mailgun", "production"]
        )
        super().__init__(metadata)
        self.api_key = os.getenv("MAILGUN_API_KEY", "")
        self.base_url = os.getenv("MAILGUN_URL", "https://api.mailgun.com")
    ...
    async def _validate(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with self.session.post(
            ...
            if response.status == 200:
                return await response.json()
            raise Exception(f"validate failed: {response.status}")
