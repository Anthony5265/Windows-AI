"""
Traefik - Production Implementation
Reverse proxy
"""
...
class TraefikPlugin(IntegrationPlugin):
    def __init__(self):
        metadata = PluginMetadata(
            ...
            tags=["traefik", "production"]
        )
        super().__init__(metadata)
        self.api_key = os.getenv("TRAEFIK_API_KEY", "")
        self.base_url = os.getenv("TRAEFIK_URL", "https://api.traefik.com")
    ...
    async def _certificate(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with self.session.post(
            ...
            if response.status == 200:
                return await response.json()
            raise Exception(f"certificate failed: {response.status}")
