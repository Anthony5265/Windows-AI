"""
pfSense - Production Implementation
Firewall
"""
...
class pfSensePlugin(IntegrationPlugin):
    def __init__(self):
        metadata = PluginMetadata(
            ...
            tags=["pfsense", "production"]
        )
        super().__init__(metadata)
        self.api_key = os.getenv("PFSENSE_API_KEY", "")
        self.base_url = os.getenv("PFSENSE_URL", "https://api.pfsense.com")
    ...
    async def _rule(self, params: Dict[str, Any]) -> Dict[str, Any]:
        """Execute rule action"""
        async with self.session.post(
            ...
            if response.status == 200:
                return await response.json()
            raise Exception(f"rule failed: {response.status}")
