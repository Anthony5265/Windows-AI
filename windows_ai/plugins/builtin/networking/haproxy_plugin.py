"""
HAProxy - Production Implementation
Load balancer
"""
...
class HAProxyPlugin(IntegrationPlugin):
    def __init__(self):
        metadata = PluginMetadata(
            ...
            tags=["haproxy", "production"]
        )
        super().__init__(metadata)
        self.api_key = os.getenv("HAPROXY_API_KEY", "")
        self.base_url = os.getenv("HAPROXY_URL", "https://api.haproxy.com")
    ...
    async def _reload(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with self.session.post(
            ...
            if response.status == 200:
                return await response.json()
            raise Exception(f"reload failed: {response.status}")
