...
class OpenCVPlugin(IntegrationPlugin):
    def __init__(self):
        metadata = PluginMetadata(
            ...
            tags=["opencv", "production"]
        )
        super().__init__(metadata)
        self.api_key = os.getenv("OPENCV_API_KEY", "")
        self.base_url = os.getenv("OPENCV_URL", "https://api.opencv.com")
    ...
    async def _recognize(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with self.session.post(
            ...
            if response.status == 200:
                return await response.json()
            raise Exception(f"recognize failed: {response.status}")
