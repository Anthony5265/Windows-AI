"""
Dalai Local Platform Integration - PRODUCTION
"""
...
class DalaiPlugin(IntegrationPlugin):
    ...
    def __init__(self):
        metadata = PluginMetadata(
            id="dalai_local",
            name="Dalai",
            description="Dalai local AI platform with full model management",
            version="2.0.0",
            author="Windows AI Team",
            plugin_type=PluginType.INTEGRATION,
            tags=["local", "ai", "llm", "dalai"],
            requirements=["aiohttp>=3.8.0", "psutil>=5.9.0"]
        )
        super().__init__(metadata)

        self.base_url = os.getenv("DALAI_URL", "http://localhost:8000")
        self.model_path = os.getenv("DALAI_MODELS", "./models")
    ...
    async def _start_server(self):
        """Start local server"""
        cmd = ["dalai", "--port", "8000", "--models-path", self.model_path]
