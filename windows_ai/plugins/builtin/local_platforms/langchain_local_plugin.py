"""
LangChain Local Local Platform Integration - PRODUCTION
"""
...
class LangChainLocalPlugin(IntegrationPlugin):
    """LangChain Local local model platform"""

    def __init__(self):
        metadata = PluginMetadata(
            id="langchain_local_local",
            name="LangChain Local",
            description="LangChain Local local AI platform with full model management",
            version="2.0.0",
            author="Windows AI Team",
            plugin_type=PluginType.INTEGRATION,
            tags=["local", "ai", "llm", "langchain local"],
            requirements=["aiohttp>=3.8.0", "psutil>=5.9.0"]
        )
        super().__init__(metadata)

        self.base_url = os.getenv("LANGCHAIN_LOCAL_URL", "http://localhost:8000")
        self.model_path = os.getenv("LANGCHAIN_LOCAL_MODELS", "./models")
    ...
    async def _start_server(self):
        """Start local server"""
        cmd = ["langchain local", "--port", "8000", "--models-path", self.model_path]
