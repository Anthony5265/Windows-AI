"""
txtai Local Platform Integration - PRODUCTION
"""
...
class txtaiPlugin(IntegrationPlugin):
    ...
    def __init__(self):
        metadata = PluginMetadata(
            id="txtai_local",
            name="txtai",
            description="txtai local AI platform with full model management",
            version="2.0.0",
            author="Windows AI Team",
            plugin_type=PluginType.INTEGRATION,
            tags=["local", "ai", "llm", "txtai"],
            requirements=["aiohttp>=3.8.0", "psutil>=5.9.0"]
        )
        super().__init__(metadata)

        self.base_url = os.getenv("TXTAI_URL", "http://localhost:8000")
        self.model_path = os.getenv("TXTAI_MODELS", "./models")
    ...
    async def _start_server(self):
        """Start local server"""
        cmd = ["txtai", "--port", "8000", "--models-path", self.model_path]
