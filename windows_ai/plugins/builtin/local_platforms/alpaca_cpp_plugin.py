"""
Alpaca.cpp Local Platform Integration - PRODUCTION
"""
...
class AlpacacppPlugin(IntegrationPlugin):
    ...
    def __init__(self):
        metadata = PluginMetadata(
            id="alpaca_cpp_local",
            name="Alpaca.cpp",
            description="Alpaca.cpp local AI platform with full model management",
            version="2.0.0",
            author="Windows AI Team",
            plugin_type=PluginType.INTEGRATION,
            tags=["local", "ai", "llm", "alpaca.cpp"],
            requirements=["aiohttp>=3.8.0", "psutil>=5.9.0"]
        )
        super().__init__(metadata)

        self.base_url = os.getenv("ALPACA_CPP_URL", "http://localhost:8000")
        self.model_path = os.getenv("ALPACA.CPP_MODELS", "./models")
    ...
    async def _start_server(self):
        """Start local server"""
        cmd = ["alpaca.cpp", "--port", "8000", "--models-path", self.model_path]
