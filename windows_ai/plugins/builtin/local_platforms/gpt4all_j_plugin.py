"""
GPT4All-J Local Platform Integration - PRODUCTION
"""
...
class GPT4AllJPlugin(IntegrationPlugin):
    ...
    def __init__(self):
        metadata = PluginMetadata(
            id="gpt4all_j_local",
            name="GPT4All-J",
            description="GPT4All-J local AI platform with full model management",
            version="2.0.0",
            author="Windows AI Team",
            plugin_type=PluginType.INTEGRATION,
            tags=["local", "ai", "llm", "gpt4all-j"],
            requirements=["aiohttp>=3.8.0", "psutil>=5.9.0"]
        )
        super().__init__(metadata)

        self.base_url = os.getenv("GPT4ALL_J_URL", "http://localhost:8000")
        self.model_path = os.getenv("GPT4ALL-J_MODELS", "./models")
    ...
    async def _start_server(self):
        """Start local server"""
        cmd = ["gpt4all-j", "--port", "8000", "--models-path", self.model_path]
