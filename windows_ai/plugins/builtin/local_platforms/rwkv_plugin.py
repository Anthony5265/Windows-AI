"""
RWKV Local Platform Integration - PRODUCTION
"""
...
class RWKVPlugin(IntegrationPlugin):
    """RWKV local model platform"""

    def __init__(self):
        metadata = PluginMetadata(
            id="rwkv_local",
            name="RWKV",
            description="RWKV local AI platform with full model management",
            version="2.0.0",
            author="Windows AI Team",
            plugin_type=PluginType.INTEGRATION,
            tags=["local", "ai", "llm", "rwkv"],
            requirements=["aiohttp>=3.8.0", "psutil>=5.9.0"]
        )
        super().__init__(metadata)

        self.base_url = os.getenv("RWKV_URL", "http://localhost:8000")
        self.model_path = os.getenv("RWKV_MODELS", "./models")
    ...
    async def _start_server(self):
        """Start local server"""
        cmd = ["rwkv", "--port", "8000", "--models-path", self.model_path]
