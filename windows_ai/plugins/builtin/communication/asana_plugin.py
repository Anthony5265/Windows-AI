"""
Asana - Production Implementation
Project management
"""
...
class AsanaPlugin(IntegrationPlugin):
    def __init__(self):
        metadata = PluginMetadata(
            ...
            tags=["asana", "production"]
        )
        super().__init__(metadata)
        self.api_key = os.getenv("ASANA_API_KEY", "")
        self.base_url = os.getenv("ASANA_URL", "https://api.asana.com")
    ...
    async def _complete(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with self.session.post(
            ...
            if response.status == 200:
                return await response.json()
            raise Exception(f"complete failed: {response.status}")
