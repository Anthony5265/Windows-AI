"""
Bitcoin - Production Implementation
Cryptocurrency
"""
...
class BitcoinPlugin(IntegrationPlugin):
    def __init__(self):
        metadata = PluginMetadata(
            ...
            tags=["bitcoin", "production"]
        )
        super().__init__(metadata)
        self.api_key = os.getenv("BITCOIN_API_KEY", "")
        self.base_url = os.getenv("BITCOIN_URL", "https://api.bitcoin.com")
    ...
    async def _balance(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with self.session.post(
            ...
            if response.status == 200:
                return await response.json()
            raise Exception(f"balance failed: {response.status}")
