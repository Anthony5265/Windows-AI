"""
OpenWeatherMap - Production Implementation
Weather API
"""
...
class OpenWeatherMapPlugin(IntegrationPlugin):
    def __init__(self):
        ...
        self.base_url = os.getenv("OPENWEATHER_URL", "https://api.openweather.com")
    ...
    async def _history(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with self.session.post(
            ...
            if response.status == 200:
                return await response.json()
            raise Exception(f"history failed: {response.status}")
