...
class Florence2Plugin(IntegrationPlugin):
    ...
    async def _unified_vision(self, params: Dict[str, Any]) -> Dict[str, Any]:
        """Unified Vision action"""
        try:
            headers = {"Authorization": f"Bearer {self.api_key}"}

            async with self.session.post(
                f"{self.base_url}/unified_vision",
                json=params,
                headers=headers,
                timeout=30
            ) as response:
                if response.status == 200:
                    ...
                    return {"result": data, "action": "unified_vision"}
                else:
                    error = await response.text()
                    raise Exception(f"Florence-2 API error {response.status}: {error}")
        except Exception as e:
