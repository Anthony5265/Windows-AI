"""
TASK-061: Windows Hello Plugin - Production Implementation
...
class WindowsHelloPlugin(IntegrationPlugin):
    """Windows Hello integration plugin"""

    def __init__(self):
        metadata = PluginMetadata(
            id="task-061_windows_hello",
            name="Windows Hello",
        ...
        self.api_key = os.getenv("WINDOWS_HELLO_API_KEY", "")
    ...
    async def connect(self, credentials: Dict[str, str]) -> bool:
        ...
        try:
            ...
            self.connected = True
            logger.info("Connected to Windows Hello")
            return True
        except Exception as e:
    ...
    async def execute(self, action: str, parameters: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        ...
        if not self.connected:
            return {"success": False, "error": "Not connected to Windows Hello"}
        ...
        try:
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    raise Exception(f"Windows Hello API error {response.status}: {error}")
        except Exception as e:
    ...
    async def _enroll(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    data = await response.json()
                    return {"result": data, "action": "enroll"}
                else:
                    error = await response.text()
                    raise Exception(f"Windows Hello API error {response.status}: {error}")
        except Exception as e:
            raise Exception(f"enroll failed: {e}")
    ...
    async def _verify(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    data = await response.json()
                    return {"result": data, "action": "verify"}
                else:
                    error = await response.text()
                    raise Exception(f"Windows Hello API error {response.status}: {error}")
        except Exception as e:
    ...
    def get_schema(self) -> Dict[str, Any]:
        ...
        return {
            "type": "object",
            "properties": {
                "action": {"type": "string", "enum": ['authenticate', 'enroll', 'verify', 'manage']},
