...
class PactPlugin(IntegrationPlugin):
        ...
        try:
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    return {"result": data, "action": "contract"}
                else:
                    error = await response.text()
                    raise Exception(f"Pact API error {response.status}: {error}")
        except Exception as e:
    ...
    async def _canideploy(self, params: Dict[str, Any]) -> Dict[str, Any]:
        """Canideploy action"""
        try:
            headers = {"Authorization": f"Bearer {self.api_key}"}

            async with self.session.post(
                f"{self.base_url}/canideploy",
                json=params,
                headers=headers,
                timeout=30
            ) as response:
                if response.status == 200:
                    data = await response.json()
                    return {"result": data, "action": "canideploy"}
                else:
                    error = await response.text()
                    raise Exception(f"Pact API error {response.status}: {error}")
        except Exception as e:
