"""
Indeed - Production Implementation
Job board
"""
...
class IndeedPlugin(IntegrationPlugin):
    def __init__(self):
        ...
        self.base_url = os.getenv("INDEED_URL", "https://api.indeed.com")
    ...
    async def _apply(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with self.session.post(
            ...
            if response.status == 200:
                return await response.json()
            raise Exception(f"apply failed: {response.status}")
