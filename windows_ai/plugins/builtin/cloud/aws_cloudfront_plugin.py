"""
AWS CloudFront - Production Implementation
CDN
"""
...
class AWSCloudFrontPlugin(IntegrationPlugin):
    def __init__(self):
        metadata = PluginMetadata(
            ...
            tags=["aws_cloudfront", "production"]
        )
        super().__init__(metadata)
        self.api_key = os.getenv("AWS_CLOUDFRONT_API_KEY", "")
        self.base_url = os.getenv("AWS_CLOUDFRONT_URL", "https://api.aws_cloudfront.com")
    ...
    async def _invalidate(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with self.session.post(
            ...
            if response.status == 200:
                return await response.json()
            raise Exception(f"invalidate failed: {response.status}")
