"""
HelloSign - Production Implementation
Digital signatures
"""
...
class HelloSignPlugin(IntegrationPlugin):
    def __init__(self):
        metadata = PluginMetadata(
            ...
            tags=["hellosign", "production"]
        )
        super().__init__(metadata)
        self.api_key = os.getenv("HELLOSIGN_API_KEY", "")
        self.base_url = os.getenv("HELLOSIGN_URL", "https://api.hellosign.com")
    ...
    async def _template(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        async with self.session.post(
            ...
            if response.status == 200:
                return await response.json()
            raise Exception(f"template failed: {response.status}")
