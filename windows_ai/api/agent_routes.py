...
class AgentCreateRequest(BaseModel):
    ...
    capabilities: Optional[List[str]] = Field(
        default=None,
        description="Agent capabilities/plugins"
    )
...
async def list_agents(
    ...
    Returns list of all agents with optional status filter.
    """
    try:
...
async def get_agent(
    ...
    return AgentResponse(
        ...
        tasks_completed=agent_dict.get('tasks_completed', 0),
...
async def delete_agent(
    ...
    Deletes an agent if it's not currently busy.
    """
    success = await manager.delete_agent(agent_id)
    if not success:
        raise HTTPException(
            status_code=400,
            detail=f"Cannot delete agent {agent_id} (not found or busy)"
        )
...
async def get_agent_tasks(
    ...
    Returns all tasks assigned to this agent.
    """
    agent = manager.get_agent(agent_id)
    if not agent:
        raise HTTPException(status_code=404, detail=f"Agent {agent_id} not found")
...
async def create_task(
    ...
    Creates a task and optionally assigns it to an agent.
    ...
    try:
        ...
        task = await manager.create_task(
            task_or_dict=request.description,
...
async def list_tasks(
    ...
    Returns list of all tasks with optional filters.
    """
...
async def execute_task(
    ...
    Triggers execution of a pending task.
    """
    task = manager.get_task(task_id)
    if not task:
        raise HTTPException(status_code=404, detail=f"Task {task_id} not found")
    ...
    if task.status != TaskStatus.PENDING:
        raise HTTPException(
            status_code=400,
            detail=f"Task {task_id} is not pending (status: {task.status.value})"
        )
...
async def create_workflow(
    ...
    Creates a workflow with multiple tasks that can be executed
    sequentially or in parallel across multiple agents.
    """
    try:
        ...
        for task_spec in request.tasks:
            task = await manager.create_task(
                task_or_dict=task_spec.get('description', ''),
                parameters=task_spec.get('parameters'),
                required_plugins=task_spec.get('required_plugins')
            )
        ...
        if request.parallel:
            ...
            async def execute_parallel():
                await asyncio.gather(*[
                    manager.execute_task(tid) for tid in task_ids
                ])
...
async def get_stats(
    ...
    Returns statistics about agents and tasks.
    """
    stats = manager.get_stats()

    return AgentStatsResponse(
        total_agents=stats['agents']['total'],
        idle_agents=stats['agents']['idle'],
        busy_agents=stats['agents']['busy'],
        ...
        in_progress_tasks=stats['tasks']['in_progress']
    )
